...
* **Offline Virtual File View (Horizon 2)**: Add an **Offline** destination to the app sidebar that presents locally retained Drive items with the browser's normal grid/list controls and search. It must remain responsive with thousands of files, represent recursively retained folders without expanding their complete subtrees into Settings, and distinguish explicit offline selections from descendants covered by an offline folder. This replaces the removed Settings list; `pdfs pins` remains the diagnostic CLI view.
